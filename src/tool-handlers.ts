...
interface ClosingCostArgs {
  ...
  address?: string;
  include_pdf?: number;
  include_property_tax?: number;
  include_section?: number;
  include_payee_info?: number;
}

interface PropertyTaxArgs {
  state: string;
  county: string;
  city: string;
  address: string;
...
interface EndorsementsArgs {
  state: string;
  county: string;
  purpose: '00' | '04' | '11';
  sub_agent_id?: number;
  sub_agent_office_id?: number;
}
...
interface SubAgentsArgs {
  ...
  township?: string;
  address?: string;
  get_contact_info?: number;
}

interface CountiesArgs {
  state: string;
}

interface TownshipsArgs {
  state: string;
  county: string;
}
...
interface AppraisalModifiersArgs {
  state: string;
  county: string;
  purpose: '00' | '04' | '11';
  prop_type?: number;
...
export class ToolHandlers {
    ...
    return {
      tools: [
        ...
        },
        {
          ...
          description:
            'Get appraisal fee modifiers based on property and loan type',
          inputSchema: {
            type: 'object',
            required: ['state', 'county', 'purpose'],
            properties: {
              ...
              prop_type: {
                ...
                maximum: 7,
                description:
                  '1=Single Family, 2=Multi, 3=Condo, 4=Coop, 5=PUD, 6=Manufactured, 7=Land',
              },
              amort_type: {
                type: 'number',
                minimum: 1,
  ...
  async handleToolCall(name: string, args: any): Promise<CallToolResult> {
    ...
    try {
      switch (name) {
        case 'login':
          return await this.handleLogin(args as LoginArgs);
        case 'closing_cost_calculations':
          return await this.handleClosingCostCalculations(
            args as ClosingCostArgs
          );
        case 'property_tax':
          return await this.handlePropertyTax(args as PropertyTaxArgs);
        case 'get_endorsements':
          return await this.handleGetEndorsements(args as EndorsementsArgs);
        case 'get_sub_agents':
          return await this.handleGetSubAgents(args as SubAgentsArgs);
        case 'get_counties':
          return await this.handleGetCounties(args as CountiesArgs);
        case 'get_townships':
          return await this.handleGetTownships(args as TownshipsArgs);
        case 'get_questions':
          return await this.handleGetQuestions(args as QuestionsArgs);
        case 'geocode_check':
          return await this.handleGeocodeCheck(args as GeocodeCheckArgs);
        case 'get_appraisal_modifiers':
          return await this.handleGetAppraisalModifiers(
            args as AppraisalModifiersArgs
          );
        default:
  ...
  private async handleLogin(args: LoginArgs): Promise<CallToolResult> {
  ...
  private async handleClosingCostCalculations(
    args: ClosingCostArgs
  ...
  private async handlePropertyTax(
    ...
    const response = await this.axiosInstance.get(
      `/property_tax.php?${params}`
    );
    return this.createSuccessResponse(response.data);
  }

  private async handleGetEndorsements(
    args: EndorsementsArgs
  ): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
      return this.createSuccessResponse({
        success: true,
    ...
    const response = await this.axiosInstance.get(
      `/endorsements.php?${params}`
    );
    return this.createSuccessResponse(response.data);
  }

  private async handleGetSubAgents(
    args: SubAgentsArgs
  ): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
      return this.createSuccessResponse({
  ...
  private async handleGetCounties(args: CountiesArgs): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
      const counties = {
      ...
      return this.createSuccessResponse({
        ...
        demo_mode: true,
        state: args.state,
        counties: counties[args.state as keyof typeof counties] || [
          'Demo County',
          'Sample County',
        ],
        total_counties: 4,
  ...
  private async handleGetTownships(
    args: TownshipsArgs
  ): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
      return this.createSuccessResponse({
  ...
  private async handleGeocodeCheck(
    args: GeocodeCheckArgs
  ): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
    ...
    const response = await this.axiosInstance.get(
      `/geocode_check.php?${params}`
    );
    return this.createSuccessResponse(response.data);
  }

  private async handleGetAppraisalModifiers(
    args: AppraisalModifiersArgs
  ): Promise<CallToolResult> {
    if (DemoMode.isEnabled()) {
      ...
      if (args.loan_type === 2) modifiers.push({ type: 'FHA', adjustment: 75 });
      ...
      return this.createSuccessResponse({
        ...
        modifiers,
        final_appraisal_fee:
          baseAppraisal + modifiers.reduce((sum, m) => sum + m.adjustment, 0),
      });
    }
    ...
    const response = await this.axiosInstance.get(
      `/appraisal_modifiers.php?${params}`
    );
    return this.createSuccessResponse(response.data);
  }
  ...
  private createSuccessResponse(data: any): CallToolResult {
    return {
      content: [
        {
          type: 'text' as const,
  ...
  private createErrorResponse(error: any): CallToolResult {
    const errorMessage =
